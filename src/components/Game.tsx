...
// GameContent component that uses context
const GameContent = () => {
        ...
        <div className="p-3 border-b border-gray-800/50">
          <div className="flex items-center justify-between bg-gray-800/40 rounded-lg p-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-gray-200">Command Level</span>
            </div>
            <span className="px-2.5 py-1 bg-red-900/50 rounded-md text-red-300 font-bold text-sm">
              Level {user?.level || 1}
            </span>
          </div>
        </div>
...
// Main Game component that provides context
const Game = ({User}: GameProps) => {
  const [initialUser] = useState(User || defaultUser);
  
  return (
    <GameProvider initialUser={initialUser}>
      <GameContent />
    </GameProvider>
  );
};
